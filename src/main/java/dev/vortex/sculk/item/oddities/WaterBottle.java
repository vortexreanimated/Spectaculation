...
public class WaterBottle implements MaterialStatistics, MaterialFunction, ItemData {
		...
		for (String k : compound.c()) {
			...
			lore.add(type.getName() + " " + SUtil.toRomanNumeral(effect.getLevel())
					+ (!effect.getType().isInstant()
							? ChatColor.WHITE + " (" + effect.getDurationDisplay() + ")"
							: ""));
	...
	public void onInstanceUpdate(SItem instance) {
		...
		if (instance.getPotionEffects().size() == 1) {
			instance.setDisplayName(ChatColor
					.stripColor(instance.getPotionEffects().getFirst().getType().getName() + " "
							+ SUtil.toRomanNumeral(instance.getPotionEffects().getFirst().getLevel()))
					+ (instance.isSplashPotion() ? " Splash" : "") + " Potion");
		}
		if (instance.getPotionEffects().size() > 1) {
