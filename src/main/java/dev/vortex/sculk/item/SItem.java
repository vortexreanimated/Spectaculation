...
import dev.vortex.sculk.potion.PotionColor;
...
public class SItem implements Cloneable, ConfigurationSerializable {
	...
	private static final List<String> GLOBAL_DATA_KEYS = Arrays.asList("type", "variant", "stack", "rarity", "origin",
			"recombobulated");
	...
	protected SItem(SMaterial type, short variant, ItemStack stack, Rarity rarity, ItemOrigin origin,
			boolean recombobulated, NBTTagCompound data, boolean overwrite) {
		this.type = type;
	...
	public String getFullName() {
		return this.rarity.getColor() + (data.hasKey("reforge")
				? ReforgeType.getReforgeType(data.getString("reforge")).getReforge().getName() + " "
	...
	public void update(boolean instanceUpdate) {
		...
		if (isPotion() && getPotionEffects().size() > 0) {
			stack.setDurability(
					isSplashPotion() ? SUtil.getTopColor(this).getSplashData() : SUtil.getTopColor(this).getData());
		}
	...
	public static SItem of(SMaterial specMaterial, short variant, ItemOrigin origin) {
		...
		if (specMaterial.getCraftMaterial() == Material.SKULL_ITEM
				&& statistics instanceof SkullStatistics skullStatistics) {
			stack.setDurability((short) 3);
			SUtil.getSkull(skullStatistics.getURL(), stack, specMaterial);
		}
		if (statistics instanceof LeatherArmorStatistics armorStatistics) {
			SUtil.applyColorToLeatherArmor(stack, Color.fromRGB(armorStatistics.getColor()));
		}
		ItemMeta meta = stack.getItemMeta();
		meta.setDisplayName(statistics.getRarity().getColor() + specMaterial.getDisplayName(variant));
		stack.setItemMeta(meta);
		return new SItem(specMaterial, variant, stack, statistics.getRarity(), origin, false,
				specMaterial.getItemData() != null ? specMaterial.getItemData().getData() : new NBTTagCompound(), true);
	}
	...
	public boolean equals(Object o) {
		...
		SItem item = (SItem) o;
		return type == item.type && variant == item.variant && stack.equals(item.stack) && rarity == item.rarity
				&& origin == item.origin && recombobulated == item.recombobulated && data.equals(item.data);
	}
	...
	public static SItem deserialize(Map<String, Object> map) {
		...
		return new SItem(material, variant,
				new ItemStack(material.getCraftMaterial(), (int) map.get("amount"), variant),
				Rarity.getRarity((String) map.get("rarity")), ItemOrigin.valueOf((String) map.get("origin")),
				(boolean) map.get("recombobulated"), data, true);
	}

	@Override
	public String toString() {
		return "SItem{type=" + type.name() + ", variant=" + variant + ", stack=" + stack.toString() + ", rarity="
				+ rarity.name() + ", origin=" + origin.name() + ", recombobulated=" + recombobulated + ", data="
				+ data.toString() + "}";
	}
	...
	public static SItem of(NBTTagCompound compound) {
		...
		ItemStack stack = new ItemStack(type.getCraftMaterial(),
				compound.hasKey("amount") ? compound.getInt("amount") : 1, type.getData());
		MaterialStatistics statistics = type.getStatistics();
		if (type.getCraftMaterial() == Material.SKULL_ITEM && statistics instanceof SkullStatistics skullStatistics) {
			stack.setDurability((short) 3);
			SUtil.getSkull(skullStatistics.getURL(), stack, type);
		}
		if (statistics instanceof LeatherArmorStatistics armorStatistics) {
			SUtil.applyColorToLeatherArmor(stack, Color.fromRGB(armorStatistics.getColor()));
		}
		...
		return new SItem(type, variant, stack,
				...
				compound.getBoolean("recombobulated"), data, true);
	}

	public static SItem convert(ItemStack stack) {
		return SUtil.setSItemAmount(SItem.of(SMaterial.getSpecEquivalent(stack.getType(), stack.getDurability())),
				stack.getAmount());
	}
	...
	private static SItem of(NBTTagCompound compound, ItemStack stack) {
		...
		return new SItem(type, compound.hasKey("variant") ? compound.getShort("variant") : 0, stack,
				(compound.hasKey("rarity")
						? Rarity.getRarity(compound.getString("rarity"))
						: type.getStatistics().getRarity()),
				compound.hasKey("origin") ? ItemOrigin.valueOf(compound.getString("origin")) : ItemOrigin.UNKNOWN,
