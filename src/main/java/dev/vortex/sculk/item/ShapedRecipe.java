...
public class ShapedRecipe extends Recipe<ShapedRecipe> {
	...
	private MaterialQuantifiable[][] toMQ2DArray() {
		...
		for (int i = 0; i < ls.length; i++) {
			...
			for (int j = 0; j < lps.length; j++) {
				materials[i][j] = ingredientMap.getOrDefault(lps[j].charAt(0),
						new MaterialQuantifiable(SMaterial.AIR, 1));
			}
		}
		return materials;
	}
	...
	protected static ShapedRecipe parseShapedRecipe(ItemStack[] stacks) {
		...
		for (ShapedRecipe recipe : CACHED_RECIPES) {
			...
			if (!recipeAccepted(recipe.useExchangeables, grid, recipeGrid)
					|| !recipeAccepted(recipe.useExchangeables, seg, recipeSeg)) {
				continue;
			}
			return recipe;
		}
	...
	private static boolean recipeAccepted(boolean usesExchangeables, MaterialQuantifiable[][] grid,
			MaterialQuantifiable[][] recipeGrid) {
		...
		try {
			for (int i = 0; i < grid.length; i++) {
				for (int j = 0; j < grid[i].length; j++) {
					...
					if (usesExchangeables && exchangeables != null && exchangeables.contains(m1.getMaterial())
	...
	private static boolean recipeAccepted(boolean usesExchangeables, MaterialQuantifiable[] grid1d,
			MaterialQuantifiable[] recipeGrid1d) {
