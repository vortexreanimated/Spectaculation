...
public class GungaPet extends Pet {
	...
	public List<PetAbility> getPetAbilities(SItem instance) {
		...
		BigDecimal annih = BigDecimal.valueOf(1.0 / (annihCh.getForRarity(instance.getRarity()) - (level * 0.02)))
		...
		if (instance.getRarity().isAtLeast(Rarity.RARE)) {
			abilities.add(new PetAbility() {
				...
				@Override
				public List<String> getDescription(SItem instance) {
					return Arrays.asList("When hurt, you will have a",
							ChatColor.GREEN + annih.toPlainString() + "%" + ChatColor.GRAY + " chance to explode.");
				}
			...
			abilities.add(new PetAbility() {
				...
				public List<String> getDescription(SItem instance) {
					return Arrays.asList(
							"Have a " + ChatColor.GREEN + pig.toPlainString() + "%" + ChatColor.GRAY + " chance",
