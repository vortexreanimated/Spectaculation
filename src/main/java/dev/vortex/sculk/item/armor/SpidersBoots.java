...
public class SpidersBoots implements ToolStatistics, TickingMaterial, FlightStatistics, Ability {
	...
	public void onAbilityUse(Player player, SItem sItem) {
		...
		player.setVelocity(
				player.getVelocity().clone().add(player.getLocation().getDirection().multiply(0.8)).setY(0.6));
	}
	...
	public void tick(SItem item, Player owner) {
		SUtil.toggleAllowFlightNoCreative(owner.getUniqueId(),
				owner.getLocation().clone().subtract(0, 0.2, 0).getBlock().getType() == Material.AIR);
	}
