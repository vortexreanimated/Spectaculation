...
import dev.vortex.sculk.item.storage.Storage;
...
public class ItemListener extends PListener {
	...
	public void onInventoryClose(InventoryCloseEvent e) // todo: rework storage
	{
		...
		for (int i = 0; i < inventory.getSize(); i++) {
			...
			if (sItem == null) {
				...
				if (equiv != null) {
					storageData.setByteArray(String.valueOf(i),
							SUtil.gzipCompress(equiv.toCompound().toString().getBytes()));
					continue;
				}
	...
	public void onInventoryClick(InventoryClickEvent e) {
		...
		if (sItem.getType().getStatistics().getSpecificType() == null
				|| sItem.getType().getStatistics().getSpecificType() != SpecificItemType.HELMET) {
			return;
		}
	...
	public void onBlockPlace(BlockPlaceEvent e) {
		...
		if (sItem.getType().getStatistics().getSpecificType() == SpecificItemType.HELMET
				&& isAir(e.getPlayer().getInventory().getHelmet())) {
			e.setCancelled(true);
			e.getPlayer().getInventory().setHelmet(sItem.getStack());
			e.getPlayer().setItemInHand(null);
			return;
	...
	public void onFrameInteract(PlayerInteractEvent e) {
		...
		if (!block.hasMetadata("placer")) {
			...
			List<Location> locations = StaticDragonManager.EYES.containsKey(player.getUniqueId())
					? StaticDragonManager.EYES.get(player.getUniqueId())
					: new ArrayList<>();
			...
			for (List<Location> ls : StaticDragonManager.EYES.values())
				quantity += ls.size();
			Bukkit.broadcastMessage(ChatColor.DARK_PURPLE + "☬ " + ChatColor.GREEN + player.getName()
					+ ChatColor.LIGHT_PURPLE + " placed a Summoning Eye! " + (quantity == 8 ? "Brace yourselves! " : "")
					+ ChatColor.GRAY + "(" + (quantity == 8 ? ChatColor.GREEN : ChatColor.YELLOW) + quantity
			...
			for (List<Location> ls : StaticDragonManager.EYES.values()) {
				for (Location location : ls) {
					...
					p.sendMessage(
							ChatColor.DARK_PURPLE + "Your Sleeping Eyes have been awoken by the magic of the Dragon."
		...
		if (item.getType() == SMaterial.SLEEPING_EYE) {
			...
			if (StaticDragonManager.ACTIVE) {
				player.sendMessage(
						ChatColor.RED + "You cannot recover Summoning Eyes after the dragon has been summoned!");
				return;
			}
	...
	public void onItemPickup(PlayerPickupItemEvent e) {
		...
		if (item.hasMetadata("owner")) {
			...
			if (o.size() != 0) {
				if (!o.getFirst().asString().equals(e.getPlayer().getUniqueId().toString())) {
					e.setCancelled(true);
					return;
				}
			}
		...
		if (item.hasMetadata("obtained")) {
			Bukkit.broadcastMessage(ChatColor.GREEN + player.getName() + ChatColor.YELLOW + " has obtained "
					+ sItem.getFullName() + ChatColor.YELLOW + "!");
		}
	...
	public void onItemDrop(PlayerDropItemEvent e) {
		...
		if (sItem != null
				&& (sItem.getType() == SMaterial.SKYBLOCK_MENU || sItem.getType() == SMaterial.QUIVER_ARROW)) {
			e.setCancelled(true);
		}
		updateStatistics(e.getPlayer());
	}
	...
	public void onPotionSplash(PotionSplashEvent e) {
		...
		for (LivingEntity entity : e.getAffectedEntities()) {
			...
			for (PotionEffect effect : item.getPotionEffects()) {
				...
				entity.sendMessage((effect.getType().isBuff()
						? ChatColor.GREEN + "" + ChatColor.BOLD + "BUFF!"
						: ChatColor.RED + "" + ChatColor.BOLD + "DEBUFF!") + ChatColor.RESET + ChatColor.WHITE + " You "
						+ (e.getPotion().getShooter().equals(entity) ? "splashed yourself" : "were splashed") + " with "
						+ effect.getDisplayName() + ChatColor.WHITE + "!");
			}
		}
	}
	...
	private static void updateStatistics(Player player) {
		...
		ItemStack beforeHelmet = inv.getHelmet(), beforeChestplate = inv.getChestplate(),
				beforeLeggings = inv.getLeggings(), beforeBoots = inv.getBoots();
		new BukkitRunnable() {
			@Override
			public void run() {
				PlayerStatistics statistics = PlayerUtils.STATISTICS_CACHE.get(player.getUniqueId());
				ItemStack afterHelmet = inv.getHelmet(), afterChestplate = inv.getChestplate(),
						afterLeggings = inv.getLeggings(), afterBoots = inv.getBoots();
				...
				if (!chestplateSimilar) {
					PlayerUtils.updateArmorStatistics(SItem.find(afterChestplate), statistics,
							PlayerStatistic.CHESTPLATE);
				}
				if (!leggingsSimilar) {
