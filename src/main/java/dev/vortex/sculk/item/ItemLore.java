...
import dev.vortex.sculk.item.orb.OrbBuff;
...
public class ItemLore {
	...
	public List<String> asBukkitLore() {
		...
		if (statistics instanceof PlayerBoostStatistics) {
			...
			if (parent.getType() == SMaterial.EMERALD_BLADE && player != null
				...
				damage = addPossiblePropertyDouble("Damage",
						playerBoostStatistics.getBaseDamage()
			...
			boolean critChance = addPossiblePropertyInt("Crit Chance",
					(int) (playerBoostStatistics.getBaseCritChance() * 100),
					(int) (reforge.getCritChance().getForRarity(parent.getRarity()) * 100), "%", false, lore);
			boolean critDamage = addPossiblePropertyInt("Crit Damage",
					(int) (playerBoostStatistics.getBaseCritDamage() * 100),
			...
			boolean magicFind = addPossiblePropertyInt("Magic Find",
					(int) (playerBoostStatistics.getBaseMagicFind() * 100), "", true, lore);
		...
		if (ability != null) {
			StringBuilder abilityTitle = new StringBuilder().append(ChatColor.GOLD).append("Item Ability: ")
					.append(ability.getAbilityName());
		...
		if (material.getItemData() != null) {
			...
			for (String key : compound.c()) {
				List<String> dl = material.getItemData().getDataLore(key,
						SUtil.getObjectFromCompound(parent.getData(), key));
				if (dl != null) {
					lore.addAll(dl);
					lore.add("");
				}
		...
		if (statistics.displayRarity()) {
			lore.add((parent.isRecombobulated()
					? parent.getRarity().getBoldedColor() + ChatColor.MAGIC + "D" + ChatColor.RESET + " "
					: "") + parent.getRarity().getDisplay()
					+ (type != SpecificItemType.NONE ? " " + type.getName() : "")
					+ (parent.isRecombobulated()
							? parent.getRarity().getBoldedColor() + " " + ChatColor.MAGIC + "D" + ChatColor.RESET
							: ""));
		}
	...
	private boolean addPossiblePropertyInt(String name, double i, int r, String succeeding, boolean green,
			List<String> list) {
		i += r;
		if (i == 0) {
			return false;
		...
		builder.append(ChatColor.GRAY).append(name).append(": ").append(green ? ChatColor.GREEN : ChatColor.RED)
	...
	private boolean addPossiblePropertyDouble(String name, double d, int r, String succeeding, boolean green,
			List<String> list) {
		d += r;
		if (d == 0.0) {
			return false;
