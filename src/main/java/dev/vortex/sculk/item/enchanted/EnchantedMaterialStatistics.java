...
public interface EnchantedMaterialStatistics extends MaterialStatistics {
	...
	default void load() {
		if (getBlockCraftingMaterial() != null && getBlockResult() != null) {
			createRecipe(new MaterialQuantifiable(getBlockCraftingMaterial(), getCraftingRequiredAmount()),
					getBlockResult());
		}
	...
	static void createRecipe(MaterialQuantifiable material, MaterialQuantifiable result) {
		new ShapelessRecipe(result.getMaterial(), result.getAmount()).add(material).add(material).add(material)
