...
import dev.vortex.sculk.item.ItemCategory;
...
public class AuctionSettings implements Cloneable, ConfigurationSerializable {
	...
	public String toString() {
		return "AuctionSettings{category=" + category.name() + ", query=" + query + ", sort=" + sort.name() + ", tier="
				+ tier.name() + ", type=" + type.name() + "}";
	}
	...
	public static AuctionSettings deserialize(Map<String, Object> map) {
		return new AuctionSettings(ItemCategory.valueOf((String) map.get("category")),
				map.containsKey("query") ? (String) map.get("query") : null, Sort.valueOf((String) map.get("sort")),
				map.containsKey("tier") ? Rarity.getRarity((String) map.get("tier")) : null,
				Type.valueOf((String) map.get("type")));
	}

	public enum Sort {
		HIGHEST_BID("Highest Bid"), LOWEST_BID("Lowest Bid"), ENDING_SOON("Ending soon"), MOST_BIDS("Most Bids");
	...
	public enum Type {
		SHOW_ALL("Show All"), BIN_ONLY("BIN Only"), AUCTIONS_ONLY("Auctions Only");
