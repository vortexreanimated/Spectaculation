...
import dev.vortex.sculk.item.Ownable;
...
import dev.vortex.sculk.sidebar.Sidebar;
				...
				for (Player player : Bukkit.getOnlinePlayers()) {
					...
					if (hand != null) {
						...
						if (hand.getType().getStatistics().getSpecificType() == SpecificItemType.BOW
								&& user.hasQuiverItem(SMaterial.ARROW)
								&& (last == null || last.getType() != SMaterial.QUIVER_ARROW)) {
							inventory.setItem(8, SUtil.setStackAmount(SItem.of(SMaterial.QUIVER_ARROW).getStack(),
					...
					if (counters[1] == 4) {
						if (player.getHealth() <= player.getMaxHealth()) {
							player.setHealth(Math.min(player.getMaxHealth(),
									player.getHealth() + 1.5 + ((int) player.getMaxHealth() * 0.01)
											+ ((1.5 + ((int) player.getMaxHealth() * 0.01))
													* statistics.getHealthRegenerationPercentBonus())));
						}
					}

					// Update Set
					PlayerUtils.updateSetStatistics(player, SItem.find(inventory.getHelmet()),
							SItem.find(inventory.getChestplate()), SItem.find(inventory.getLeggings()),
							SItem.find(inventory.getBoots()), statistics);
					...
					if (set instanceof TickingSet tickingSet) {
						tickingSet.tick(player, SItem.find(inventory.getHelmet()),
								SItem.find(inventory.getChestplate()), SItem.find(inventory.getLeggings()),
								SItem.find(inventory.getBoots()), Repeater.this.counters);
					}
					...
					sidebar.add(ChatColor.GRAY + " " + (hours > 12 ? hours - 12 : (hours == 0 ? 12 : hours)) + ":"
							+ SUtil.zeroed(minutes) + (hours >= 12 ? "pm" : "am") + " "
							+ (day ? ChatColor.YELLOW + "☀" : ChatColor.AQUA + "☽"));
					...
					if (quest != null && quest.getDied() == 0) {
						...
						if (quest.getKilled() != 0) {
							...
							sidebar.add(ChatColor.YELLOW + " " + SUtil.commaify(
									(int) (quest.getXp() / quest.getLastKilled().getStatistics().getXPDropped()))
									+ ChatColor.GRAY + "/" + ChatColor.RED
									+ SUtil.commaify((int) (quest.getType().getSpawnXP()
											/ quest.getLastKilled().getStatistics().getXPDropped()))
									+ ChatColor.GRAY + " Kills");
						} else {
							sidebar.add(ChatColor.GRAY + " (" + ChatColor.YELLOW + SUtil.commaify((int) quest.getXp())
									+ ChatColor.GRAY + "/" + ChatColor.RED
									+ SUtil.commaify(quest.getType().getSpawnXP()) + ChatColor.GRAY + ") Combat XP");
						}
						sidebar.add("    ");
					}
					if (StaticDragonManager.ACTIVE && StaticDragonManager.DRAGON != null && user.getLastRegion() != null
							&& (user.getLastRegion().getType() == RegionType.THE_END
									|| user.getLastRegion().getType() == RegionType.THE_END_NEST
									|| user.getLastRegion().getType() == RegionType.DRAGONS_NEST)) {
						sidebar.add("Dragon HP: " + ChatColor.GREEN
								+ SUtil.commaify((int) StaticDragonManager.DRAGON.getEntity().getHealth())
								+ ChatColor.RED + "❤");
