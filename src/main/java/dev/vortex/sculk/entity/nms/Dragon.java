...
public abstract class Dragon extends EntityEnderDragon implements SNMSEntity, EntityFunction, EntityStatistics {
	...
	public boolean tick(LivingEntity entity) {
		...
		if (location.getX() < -718.0 || location.getX() > -623.0 || location.getZ() < -328.0
				|| location.getZ() > -224.0) {
							...
							public void run() {
								...
								for (Entity e : entity.getNearbyEntities(200, 200, 200)) {
									...
									if (le instanceof Player) {
										User.getUser(le.getUniqueId()).damage(damage,
												EntityDamageEvent.DamageCause.ENTITY_ATTACK, entity);
									} else {
										le.damage(damage);
									}
					...
					case 1 : {
						...
						if (near != null) {
							SUtil.runIntervalForTicks(() -> {
								...
								for (int i = 0; i < 15; i++) {
									entity.getWorld().spigot().playEffect(
											entity.getEyeLocation().subtract(0.0, 8.0, 0.0)
													.add(entity.getLocation().getDirection().multiply(-5.0))
													.add(SUtil.random(-0.5, 0.5), SUtil.random(-0.5, 0.5),
															SUtil.random(-0.5, 0.5)),
							...
							new BukkitRunnable() {
								...
								public void run() {
									SUtil.runIntervalForTicks(() -> {
										...
										Fireball fireball = entity.getWorld()
												.spawn(entity.getEyeLocation().subtract(0.0, 8.0, 0.0)
														.add(entity.getLocation().getDirection().multiply(-5.0)),
														Fireball.class);
										fireball.setMetadata("dragon",
												new FixedMetadataValue(Spectaculation.getPlugin(), sEntity));
										fireball.setDirection(
												finalNear.getLocation().getDirection().multiply(-1.0).normalize());
									}, 5, 60);
								}
	...
	public void onDeath(SEntity sEntity, Entity killed, Entity damager) {
		...
		if (name != null) {
			message.append(ChatColor.GREEN).append(name).append(ChatColor.GRAY).append(" dealt the final blow.\n ");
		}
		for (int i = 0; i < Math.min(3, damageDealt.size()); i++) {
			...
			message.append(" Damager").append(ChatColor.RESET).append(ChatColor.GRAY).append(" - ")
					.append(ChatColor.GREEN).append(Bukkit.getOfflinePlayer(d.getKey()).getName())
					.append(ChatColor.GRAY).append(" - ").append(ChatColor.YELLOW)
					.append(SUtil.commaify(d.getValue().intValue()));
		}
		message.append("\n").append(ChatColor.RESET).append(ChatColor.YELLOW).append("Your Damage: ").append("%s")
		...
		new BukkitRunnable() {
			...
			public void run() {
				for (int i = 0; i < damageDealt.size(); i++) {
					...
					SUtil.addIf(new DragonDrop(SMaterial.ASPECT_OF_THE_DRAGONS, 450), possibleMajorDrops,
							weight >= 450);
					SUtil.addIf(new DragonDrop(SMaterial.VagueEntityMaterial.HELMET, 400, type), possibleMajorDrops,
							weight >= 400);
					SUtil.addIf(new DragonDrop(SMaterial.VagueEntityMaterial.CHESTPLATE, 325, type), possibleMajorDrops,
							weight >= 325);
					SUtil.addIf(new DragonDrop(SMaterial.VagueEntityMaterial.LEGGINGS, 350, type), possibleMajorDrops,
							weight >= 350);
					SUtil.addIf(new DragonDrop(SMaterial.VagueEntityMaterial.BOOTS, 300, type), possibleMajorDrops,
							weight >= 300);
					...
					List<DragonDrop> minorDrops = new ArrayList<>(
							Arrays.asList(new DragonDrop(SMaterial.ENDER_PEARL, 0),
									new DragonDrop(SMaterial.ENCHANTED_ENDER_PEARL, 0)));
					SUtil.addIf(new DragonDrop(SMaterial.VagueEntityMaterial.FRAGMENT, 22, type), minorDrops,
							weight >= 22);
					int frags = 0;
					...
					for (DragonDrop minorDrop : minorDrops) {
						...
						if (minorDrop.getVagueEntityMaterial() != null && frags != 0) {
							Item item = SUtil.spawnPersonalItem(SUtil.setStackAmount(sItem.getStack(), frags),
	...
	public void onAttack(EntityDamageByEntityEvent e) {
		...
		e.getEntity()
				.sendMessage(ChatColor.DARK_PURPLE + "☬ " + ChatColor.RED + getEntityName() + ChatColor.LIGHT_PURPLE
						+ " used " + ChatColor.YELLOW + "Rush" + ChatColor.LIGHT_PURPLE + " on you for " + ChatColor.RED
						+ d + " damage.");
	}
