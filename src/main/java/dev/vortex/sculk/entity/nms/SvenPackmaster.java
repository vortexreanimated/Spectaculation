...
public class SvenPackmaster extends EntityWolf implements SNMSEntity, EntityFunction, WolfStatistics, SlayerBoss {
	private static final TieredValue<Double> MAX_HEALTH_VALUES = new TieredValue<>(2000.0, 40000.0, 750000.0,
			2000000.0);
	...
	public SvenPackmaster(Integer tier, UUID spawnerUUID) {
		super(((CraftWorld) Bukkit.getWorlds().getFirst()).getHandle());
		this.tier = tier;
	...
	public void t_() {
		...
		this.getBukkitEntity().getWorld().spigot().playEffect(this.getBukkitEntity().getLocation(), Effect.FLAME, 0, 1,
				(float) SUtil.random(-1.0, 1.0), 0.0f, (float) SUtil.random(-1.0, 1.0), 0.0f, 1, 16);
		this.getBukkitEntity().getWorld().spigot().playEffect(this.getBukkitEntity().getLocation(),
				Effect.FIREWORKS_SPARK, 0, 1, (float) SUtil.random(-1.0, 1.0), 0.0f, (float) SUtil.random(-1.0, 1.0),
				0.0f, 1, 16);
		((Wolf) this.getBukkitEntity()).setTarget(player);
		hologram.getEntity().teleport(getBukkitEntity().getLocation().clone().add(0, 1.3, 0));
		hologram.getEntity()
				.setCustomName(ChatColor.RED + SUtil.getFormattedTime(end - System.currentTimeMillis(), 1000));
	}
	...
	public void onAttack(EntityDamageByEntityEvent e) {
		...
		User.getUser(player.getUniqueId()).damage(TRUE_DAMAGE_VALUES.getByNumber(tier),
				EntityDamageEvent.DamageCause.ENTITY_ATTACK, this.getBukkitEntity());
		...
		if (tier >= 3 && sEntity.getEntity().getHealth() - damage.get() < getEntityMaxHealth() / 2.0 && !pupsSpawned) // pups
		{
			...
			for (int i = 0; i < 10; i++) {
				SUtil.delay(() -> {
					...
					pups.add(new SEntity(sEntity.getEntity(), SEntityType.SVEN_PUP, getEntityMaxHealth() * 0.1,
							getDamageDealt() * 0.5, player, this));
				}, i * 20);
			}
		}
	}
