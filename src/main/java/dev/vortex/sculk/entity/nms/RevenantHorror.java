...
public class RevenantHorror extends EntityZombie implements SNMSEntity, EntityFunction, ZombieStatistics, SlayerBoss {
	...
	public void t_() {
		...
		if (tier >= 3 && !enraged && SUtil.random(0, 200) == 0) // enrage
		{
			...
			new BukkitRunnable() {
				...
				public void run() {
					enraged = false;
					RevenantHorror.this.getAttributeInstance(GenericAttributes.MOVEMENT_SPEED)
							.setValue(getMovementSpeed());
				}
	...
	public SEntityEquipment getEntityEquipment() {
		return new SEntityEquipment(new ItemStack(Material.DIAMOND_HOE),
				SItem.of(SMaterial.REVENANT_HORROR_HEAD).getStack(), new ItemStack(Material.DIAMOND_CHESTPLATE),
