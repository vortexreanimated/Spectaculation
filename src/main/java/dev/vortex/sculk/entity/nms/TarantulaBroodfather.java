...
public class TarantulaBroodfather extends EntitySpider
		implements
			SNMSEntity,
			EntityFunction,
			EntityStatistics,
	...
	public void onSpawn(LivingEntity entity, SEntity sEntity) {
		...
		if (tier >= 2) // noxious
		{
			new BukkitRunnable() {
				...
				public void run() {
					...
					if (player.getLocation().distance(bukkitEntity.getLocation()) > 5) {
						return;
					}
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				Vector vector = e.getLocation().clone().toVector().subtract(player.getLocation().clone().toVector())
						.multiply(-1.0).multiply(new Vector(0.1, 0.2, 0.1));
	...
	public List<EntityDrop> drops() {
		...
		if (tier >= 2) {
			...
			drops.add(new EntityDrop(
					SUtil.setStackAmount(SItem.of(SMaterial.TOXIC_ARROW_POISON).getStack(), toxicArrowPoison),
	...
	public static class TopCaveSpider implements EntityStatistics, EntityFunction {
		...
		public boolean tick(LivingEntity entity) {
			...
			entity.setCustomName(ChatColor.RED + parent.getEntityName() + " " + ChatColor.GREEN
					+ (int) (taran.getHealth()) + ChatColor.WHITE + "/" + ChatColor.GREEN + (int) taran.getMaxHealth()
					+ ChatColor.RED + "❤");
			return true;
		}
