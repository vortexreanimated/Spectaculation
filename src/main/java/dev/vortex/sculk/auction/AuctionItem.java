...
public class AuctionItem {
	...
	private AuctionItem(UUID uuid, SItem item, long starter, long end, UUID owner, List<UUID> participants,
			boolean bin) {
		this.uuid = uuid;
		this.item = item;
	...
	public AuctionBid getRecentBid() {
		return bids.size() == 0 ? null : bids.getLast();
	}
	...
	public void bid(User user, long amount) {
		...
		if (bin) {
			messageOwner(ChatColor.GOLD + "[Auction] " + ChatColor.GREEN + bidder + ChatColor.YELLOW + " bought "
					+ item.getFullName() + ChatColor.YELLOW + " for " + ChatColor.GOLD + SUtil.commaify(starter)
					+ " coin" + (starter != 1 ? "s" : ""));
			end();
		...
		messageOwner(ChatColor.GOLD + "[Auction] " + ChatColor.GREEN + bidder + ChatColor.YELLOW + " bid "
				+ ChatColor.GOLD + SUtil.commaify(starter) + " coin" + (starter != 1 ? "s" : "") + ChatColor.YELLOW
				+ " on " + item.getFullName());
		for (UUID participant : participants) {
			...
			player.sendMessage(ChatColor.GOLD + "[Auction]" + ChatColor.GREEN + bidder + ChatColor.YELLOW
					+ " outbid you by " + ChatColor.GOLD + diff + " coin" + (diff != 1 ? "s" : "") + ChatColor.YELLOW
					+ " for " + item.getFullName());
		}
	}
	...
	public String toString() {
		return "AuctionItem{uuid=" + uuid.toString() + ", item=" + item.toString() + ", bids=" + bids.toString()
				+ ", end=" + end + "}";
	}

	public static AuctionItem createAuction(SItem item, long starter, long end, UUID owner, boolean bin) {
		return new AuctionItem(UUID.randomUUID(), item, starter, end, owner,
				new ArrayList<>(Collections.singletonList(owner)), bin);
	}
	...
	public static CompletableFuture<List<AuctionItem>> search(AuctionSettings settings) {
		return CompletableFuture.supplyAsync(() -> {
			...
			// filter for category
			items = items
					.filter(item -> item.getItem().getType().getStatistics().getCategory() == settings.getCategory());
			// filter for query
			if (settings.getQuery() != null) {
				items = items.filter(item -> {
					String query = settings.getQuery().toLowerCase();
					String name = item.getItem().getType().getDisplayName(item.getItem().getType().getData())
							.toLowerCase();
					String lore = item.getItem().getLore().asBukkitLore().toString().toLowerCase();
					return query.contains(name) || query.contains(lore)
							|| query.contains(item.getItem().getType().name().toLowerCase());
				});
			}
			// sort
