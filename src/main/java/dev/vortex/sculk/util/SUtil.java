...
public class SUtil {
	...
	private static final List<ChatColor> CRIT_SPECTRUM = Arrays.asList(ChatColor.WHITE, ChatColor.WHITE,
			ChatColor.YELLOW, ChatColor.GOLD, ChatColor.RED, ChatColor.RED);
	private static final List<ChatColor> VISIBLE_COLOR_SPECTRUM = Arrays.asList(ChatColor.DARK_GREEN,
			ChatColor.DARK_AQUA, ChatColor.DARK_RED, ChatColor.DARK_PURPLE, ChatColor.GOLD, ChatColor.GREEN,
	...
	public static ItemStack getSkull(String texture, ItemStack stack, SMaterial material) {
		...
		byte[] ed = Base64.getEncoder().encode(
				"{textures:{SKIN:{url:\"http://textures.minecraft.net/texture/%s\"}}}".formatted(texture).getBytes());
	...
	public static GenericItemType getItemType(Material material) {
		...
		if (Groups.PICKAXES.contains(material) || Groups.HOES.contains(material) || Groups.AXES.contains(material)
				|| Groups.SHOVELS.contains(material)) {
			return GenericItemType.TOOL;
		}
		if (Groups.LEATHER_ARMOR.contains(material) || Groups.IRON_ARMOR.contains(material)
				|| Groups.GOLD_ARMOR.contains(material) || Groups.DIAMOND_ARMOR.contains(material)) {
	...
	public static void border(Inventory inventory, GUI gui, ItemStack stack, int cornerSlot, int cornerSlot2,
			boolean overwrite, boolean pickup) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				if (target.get() == null) {
					...
					for (Entity entity : projectile.getNearbyEntities(2 * aiming.getLevel(), 2 * aiming.getLevel(),
							2 * aiming.getLevel())) {
						if (entity instanceof Player) {
							continue;
						}
						if (entity instanceof LivingEntity livingEntity && !(entity instanceof ArmorStand)
								&& !(entity instanceof NPC) && !entity.isDead()) {
				...
				Vector vector = location.clone().toVector()
						.subtract(target.get().getLocation().clone().add(0, 1, 0).toVector());
	...
	public static boolean isEnchantable(SItem sItem) {
		...
		return type == GenericItemType.WEAPON || type == GenericItemType.TOOL || type == GenericItemType.RANGED_WEAPON
				|| type == GenericItemType.ARMOR;
	}
	...
	public static void setBlocks(Location c1, Location c2, Material material, boolean applyPhysics) {
		...
		int sy = Math.min(c1.getBlockY(), c2.getBlockY()), ey = Math.max(c1.getBlockY(), c2.getBlockY()),
				sx = Math.min(c1.getBlockX(), c2.getBlockX()), ex = Math.max(c1.getBlockX(), c2.getBlockX()),
				sz = Math.min(c1.getBlockZ(), c2.getBlockZ()), ez = Math.max(c1.getBlockZ(), c2.getBlockZ());
	...
	public static GameProfile createGameProfile(String url) {
		...
		byte[] ed = Base64.getEncoder().encode(
				"{textures:{SKIN:{url:\"http://textures.minecraft.net/texture/%s\"}}}".formatted(url).getBytes());
		profile.getProperties().put("textures", new Property("textures", new String(ed)));
		return profile;
	}
	...
	public static String createProgressText(String text, int current, int max) {
		...
		return ChatColor.GRAY + text + ": "
				+ (percent < 100.0
						? ChatColor.YELLOW + "" + SUtil.commaify(percent) + ChatColor.GOLD + "%"
						: ChatColor.GREEN + "100.0%");
	}
