	...
	public void onOpen(GUIOpenEvent e) {
		...
		set(new GUIClickableItem() {
			...
			public ItemStack getItem() {
				if (user.getAuctionEscrow().getItem() != null) {
					...
					meta.setDisplayName(
							ChatColor.GREEN + "" + ChatColor.BOLD + ChatColor.UNDERLINE + "AUCTION FOR ITEM:");
					List<String> lore = new ArrayList<>(Collections.singletonList(" "));
					lore.add(ChatColor.GRAY + "" + display.getStack().getAmount() + "x "
							+ escrow.getItem().getFullName());
				...
				return SUtil.getStack(ChatColor.YELLOW + "Click an item in your inventory!", Material.STONE_BUTTON,
						(short) 0, 1, ChatColor.GRAY + "Selects it for auction");
			}
		});
		set(new GUIClickableItem() {
			@Override
			...
			public ItemStack getItem() {
				...
				if (escrow.getItem() == null) {
					...
					lore.add(" ");
					lore.add(ChatColor.GRAY + "Item: " + escrow.getItem().getStack().getAmount() + "x "
							+ escrow.getItem().getFullName());
					lore.add(ChatColor.GRAY + "Auction duration: " + ChatColor.YELLOW
							+ SUtil.getAuctionSetupFormattedTime(escrow.getDuration()));
					lore.add(ChatColor.GRAY + "Starting bid: " + ChatColor.GOLD + SUtil.commaify(escrow.getStarter())
							+ " coins");
					lore.add(" ");
					lore.add(ChatColor.GRAY + "Creation fee: " + ChatColor.GOLD
							+ SUtil.commaify(escrow.getCreationFee(user.isAuctionCreationBIN())) + " coins");
					lore.add(" ");
					lore.add(ChatColor.YELLOW + "Click to submit!");
				}
				return SUtil.getStack((escrow.getItem() != null ? ChatColor.GREEN : ChatColor.RED) + "Create Auction",
						Material.STAINED_CLAY, (escrow.getItem() != null ? (short) 13 : (short) 14), 1, lore);
			}
		});
		set(new GUIQueryItem() {
			...
			public ItemStack getItem() {
				...
				lore.add(" ");
				lore.add(ChatColor.GRAY + "Extra fee: " + ChatColor.GOLD + "+"
						+ SUtil.commaify(escrow.getCreationFee(user.isAuctionCreationBIN())) + " coins "
						+ ChatColor.YELLOW + "(" + (bin ? 1 : 5) + "%)");
				lore.add(" ");
				lore.add(ChatColor.YELLOW + "Click to edit!");
				return SUtil.getStack(
						ChatColor.WHITE + (bin ? "Item price: " : "Starting bid: ") + ChatColor.GOLD
								+ SUtil.commaify(escrow.getStarter()) + " coins",
						bin ? Material.GOLD_INGOT : Material.POWERED_RAIL, (short) 0, 1, lore);
			}
		});
		set(new GUIClickableItem() {
			@Override
		...
		set(new GUIClickableItem() {
			...
			public ItemStack getItem() {
				...
				return SUtil.getStack(ChatColor.GREEN + "Switch to BIN", Material.GOLD_INGOT, (short) 0, 1,
						...
						ChatColor.DARK_GRAY + "(BIN means Buy It Now)", " ", ChatColor.YELLOW + "Click to switch!");
			}
		});
	}
