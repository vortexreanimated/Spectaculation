...
public class AuctionViewGUI extends GUI {
	...
	public void onOpen(GUIOpenEvent e) {
		...
		if (ret != null) {
			set(GUIClickableItem.createGUIOpenerItem(ret, player, ChatColor.GREEN + "Go Back", 49, Material.ARROW,
		...
		if (item.isExpired()) {
			...
			items.add(new GUIClickableItem() {
				...
				public ItemStack getItem() {
					...
					if (item.getBids().size() == 0) {
						...
						lore.add(ChatColor.GRAY + "Item sold to " + ChatColor.GREEN
								+ Bukkit.getOfflinePlayer(item.getTopBidder().getUuid()));
						lore.add(ChatColor.GRAY + "for " + ChatColor.GOLD + topBid + " coin" + (topBid != 1 ? "s" : "")
								+ ChatColor.GRAY + "!");
						lore.add(" ");
						lore.add(ChatColor.YELLOW + "Click to collect coins!");
					}
					return SUtil.getStack(ChatColor.GOLD + "Collect Auction",
							item.getBids().size() != 0 ? Material.GOLD_BLOCK : Material.GOLD_NUGGET, (short) 0, 1,
							lore);
				}
			});
		} else {
			items.add(new GUIClickableItem() {
				...
				public ItemStack getItem() {
					...
					lore.add(" ");
					if (item.isBin()) {
						lore.add(ChatColor.GRAY + "Price: " + ChatColor.GOLD + SUtil.commaify(item.getStarter())
								+ " coin" + (item.getStarter() != 1 ? "s" : ""));
					} else {
						lore.add(ChatColor.GRAY + "New bid: " + ChatColor.GOLD + SUtil.commaify(bid) + " coin"
								+ (bid != 1 ? "s" : ""));
						AuctionBid bid = item.getBid(user);
						if (bid != null) {
							lore.add(ChatColor.GRAY + "Your previous bid: " + ChatColor.YELLOW
									+ SUtil.commaify(bid.getAmount()) + " coin" + (bid.getAmount() != 1 ? "s" : ""));
						}
					}
					lore.add(" ");
					User top = item.getTopBidder();
					...
					return SUtil.getStack(ChatColor.GOLD + (item.isBin() ? "Buy Item Right Now" : "Submit Bid"), icon,
							(short) 0, 1, lore);
				}
			});
		}
	...
	private List<GUIItem> getAuctionItems(Player player) {
			...
			items.add(new GUIClickableItem() {
				...
				public ItemStack getItem() {
					...
					return SUtil.getStack(ChatColor.GOLD + (item.isBin() ? "Buy Item Right Now" : "Submit Bid"),
							user.getCoins() < bid || personal ? Material.POTATO_ITEM : Material.GOLD_NUGGET, (short) 0,
							1, lore);
				}
			});
		}
		...
		if (!personal && !item.isExpired() && item.nextBid() <= user.getCoins()) {
			items.add(new GUIQueryItem() {
				...
				public ItemStack getItem() {
					...
					return SUtil.getStack(ChatColor.WHITE + "Bid Amount: " + display, Material.GOLD_INGOT, (short) 0, 1,
							ChatColor.GRAY + "You need to bid at least", display + ChatColor.GRAY + " to hold",
							ChatColor.GRAY + "the top bid on this", ChatColor.GRAY + "auction.", " ",
							ChatColor.GRAY + "The " + ChatColor.YELLOW + "top bid" + ChatColor.GRAY + " on auction",
							ChatColor.GRAY + "ends wins the item.", " ", ChatColor.GRAY + "If you do not win, you can",
		...
		List<String> lore = new ArrayList<>();
		if (item.getBids().size() > 0) {
			lore.add(ChatColor.GRAY + "Total bids: " + ChatColor.GREEN + item.getBids().size() + " bid"
					+ (item.getBids().size() != 1 ? "s" : ""));
			for (int i = item.getBids().size() - 1; i >= 0; i--) {
				AuctionBid bid = item.getBids().get(i);
				lore.add(ChatColor.GRAY + "" + ChatColor.STRIKETHROUGH + "------------");
				lore.add(ChatColor.GRAY + "Bid: " + ChatColor.GOLD + bid.getAmount() + " coin"
						+ (bid.getAmount() != 1 ? "s" : ""));
				lore.add(
						ChatColor.GRAY + "By: " + ChatColor.GREEN + Bukkit.getOfflinePlayer(bid.getBidder()).getName());
				lore.add(
						ChatColor.AQUA + SUtil.getAuctionSetupFormattedTime(bid.timeSinceBid()).toLowerCase() + " ago");
			}
		} else {
	...
	/*
	 * private List<GUIItem> getFirstItem(Player player) { User user =
	 * User.getUser(player.getUniqueId()); boolean personal =
	 * item.getOwner().getUuid().equals(user.getUuid()); List<GUIItem> items = new
	 * ArrayList<>(); if (personal && item.isExpired()) { items.add(new
	 * GUIClickableItem() {
	 * 
	 * @Override public void run(InventoryClickEvent e) { // todo: collect auction }
	 * 
	 * @Override public int getSlot() { return item.isBin() ? 31 : 29; }
	 * 
	 * @Override public ItemStack getItem() { List<String> lore = new ArrayList<>();
	 * lore.add(" "); if (item.getBids().size() == 0) { if (item.isBin())
	 * lore.add(ChatColor.GRAY + "No one has bought your item."); else
	 * lore.add(ChatColor.GRAY + "No one has bid on your item.");
	 * lore.add(ChatColor.GREEN + "You may pick it back up."); lore.add(" ");
	 * lore.add(ChatColor.YELLOW + "Click to pick up item!"); } else { long topBid =
	 * item.getTopBid(); lore.add(ChatColor.GRAY + "Item sold to " + ChatColor.GREEN
	 * + Bukkit.getOfflinePlayer(item.getTopBidder().getUuid()));
	 * lore.add(ChatColor.GRAY + "for " + ChatColor.GOLD + topBid + " coin" +
	 * (topBid != 1 ? "s" : "") + ChatColor.GRAY + "!"); lore.add(" ");
	 * lore.add(ChatColor.YELLOW + "Click to collect coins!"); } return
	 * SUtil.getStack(ChatColor.GOLD + "Collect Auction", item.getBids().size() != 0
	 * ? Material.GOLD_BLOCK : Material.GOLD_NUGGET, (short) 0, 1, lore); } }); }
	 * else { items.add(new GUIClickableItem() {
	 * 
	 * @Override public void run(InventoryClickEvent e) {
	 * 
	 * }
	 * 
	 * @Override public int getSlot() { return item.isBin() ? 31 : 29; } }); } if
	 * (!item.isBin()) { if (personal && item.getBids().size() == 0) { items.add(new
	 * GUIClickableItem() {
	 * 
	 * @Override public void run(InventoryClickEvent e) {
	 * 
	 * }
	 * 
	 * @Override public int getSlot() { return 33; }
	 * 
	 * @Override public ItemStack getItem() { return SUtil.getStack(ChatColor.RED +
	 * "Cancel Auction", Material.STAINED_CLAY, (short) 14, 1, ChatColor.GRAY +
	 * "You may cancel auctions as", ChatColor.GRAY + "long as they have " +
	 * ChatColor.RED + "0" + ChatColor.GRAY + " bids!", " ", ChatColor.YELLOW +
	 * "Click to cancel auction!"); } }); } else { List<String> lore = new
	 * ArrayList<>(); if (item.getBids().size() > 0) { lore.add(ChatColor.GRAY +
	 * "Total bids: " + ChatColor.GREEN + item.getBids().size() + " bid" +
	 * (item.getBids().size() != 1 ? "s" : "")); for (int i = item.getBids().size()
	 * - 1; i >= 0; i--) { AuctionBid bid = item.getBids().get(i);
	 * lore.add(ChatColor.GRAY + "" + ChatColor.STRIKETHROUGH + "------------");
	 * lore.add(ChatColor.GRAY + "Bid: " + ChatColor.GOLD + bid.getAmount() +
	 * " coin" + (bid.getAmount() != 1 ? "s" : "")); lore.add(ChatColor.GRAY +
	 * "By: " + ChatColor.GREEN +
	 * Bukkit.getOfflinePlayer(bid.getBidder()).getName()); lore.add(ChatColor.AQUA
	 * + SUtil.getAuctionSetupFormattedTime(bid.timeSinceBid()).toLowerCase() +
	 * " ago"); } } else { lore.add(ChatColor.GRAY + "No bids have been placed on");
	 * lore.add(ChatColor.GRAY + "this item yet."); lore.add(" ");
	 * lore.add(ChatColor.GRAY + "Be the first to bid on it!"); } items.add(new
	 * GUIItem() {
	 * 
	 * @Override public int getSlot() { return 33; }
	 * 
	 * @Override public ItemStack getItem() { return SUtil.getStack(ChatColor.WHITE
	 * + "Bid History", Material.MAP, (short) 0, 1, lore); } }); } } }
	 * 
	 */
}
