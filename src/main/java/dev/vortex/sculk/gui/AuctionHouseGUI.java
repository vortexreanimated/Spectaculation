	...
	public void onOpen(GUIOpenEvent e) {
		...
		set(new GUIClickableItem() {
			...
			public ItemStack getItem() {
				return SUtil.getStack(ChatColor.GOLD + "Auctions Browser", Material.GOLD_BLOCK, (short) 0, 1,
						ChatColor.GRAY + "Find items for sale by", ChatColor.GRAY + "players across SkyBlock!", " ",
						ChatColor.GRAY + "Items offered here are for",
						ChatColor.GOLD + "auction" + ChatColor.GRAY + ", meaning you",
						ChatColor.GRAY + "have to place the top bid", ChatColor.GRAY + "to acquire them!", " ",
		...
		set(new GUIClickableItem() {
			...
			public ItemStack getItem() {
				...
				} else if (bids.size() == 1 && bids.getFirst().getTopBidder().getUuid().equals(user.getUuid())) {
					lore.add(ChatColor.GRAY + "You have the " + ChatColor.GREEN + "top bid" + ChatColor.GRAY + " on");
					lore.add(ChatColor.GRAY + "a pending auction.");
				} else {
					lore.add(ChatColor.GRAY + "You placed " + ChatColor.GREEN + bids.size() + " bid"
							+ (bids.size() != 1 ? "s" : "") + ChatColor.GRAY + " on");
					lore.add(ChatColor.GRAY + "pending auctions.");
					lore.add(" ");
					lore.add(ChatColor.GRAY + "You hold the top bid on");
					lore.add(ChatColor.GRAY + "(" + color + held + ChatColor.GRAY + "/" + color + bids.size()
							+ ChatColor.GRAY + ") of these");
				...
				return SUtil.getStack((bids.size() == 0 ? ChatColor.YELLOW : ChatColor.GREEN) + "View Bids",
						Material.GOLDEN_CARROT, (short) 0, 1, lore);
			}
		});
		...
		set(new GUIClickableItem() {
			...
			public ItemStack getItem() {
				...
				if (auctions.size() == 0) {
					...
					lore.add(ChatColor.GRAY + "You own " + ChatColor.YELLOW + auctions.size() + " auction"
							+ (auctions.size() != 1 ? "s" : "") + ChatColor.GRAY + " in");
					...
					lore.add(ChatColor.GRAY + "or typing " + ChatColor.GREEN + "/ah " + player.getName()
							+ ChatColor.GRAY + "!");
					lore.add(" ");
					lore.add(ChatColor.YELLOW + "Click to manage!");
				}
				return SUtil.getStack(ChatColor.GREEN + (auctions.size() == 0 ? "Create Auction" : "Manage Auctions"),
						Material.GOLD_BARDING, (short) 0, 1, lore);
			}
		});
		set(new GUIClickableItem() {
			@Override
			...
			public ItemStack getItem() {
				return SUtil.getStack(ChatColor.GREEN + "Auction Stats", Material.EMPTY_MAP, (short) 0, 1,
						ChatColor.GRAY + "View various statistics", ChatColor.GRAY + "about you and the Auction",
						ChatColor.GRAY + "House.", " ", ChatColor.RED + "Coming soon!");
			}
		});
	}
}
