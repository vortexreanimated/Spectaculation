...
public class PotionEffectType {
	...
	public static final PotionEffectType STRENGTH = new PotionEffectType(ChatColor.DARK_RED + "Strength", "strength",
			...
			((statistics, slot,
					level) -> statistics.getStrength().add(slot, SUtil.getOrDefault(
							Arrays.asList(5.0, 12.0, 20.0, 30.0, 40.0, 50.0, 60.0, 75.0), level - 1, level * 10.0))),
			false, true);
	...
	public static final PotionEffectType RABBIT = new PotionEffectType(ChatColor.GREEN + "Rabbit", "rabbit",
			...
			((effect, player) -> PlayerUtils.replacePotionEffect(player,
	...
	public static final PotionEffectType HEALING = new PotionEffectType(ChatColor.RED + "Healing", "healing",
			...
			((effect,
					player) -> player.setHealth(Math.min(player.getMaxHealth(),
							player.getHealth() + SUtil.getOrDefault(Arrays.asList(20, 50, 100, 150, 200, 300, 400, 500),
									effect.getLevel() - 1, (effect.getLevel() - 3) * 100)))),
			true, true);
	...
	public static final PotionEffectType JUMP_BOOST = new PotionEffectType(ChatColor.AQUA + "Jump Boost", "jump_boost",
			...
			((effect, player) -> PlayerUtils.replacePotionEffect(player, new org.bukkit.potion.PotionEffect(
					org.bukkit.potion.PotionEffectType.JUMP, (int) effect.getDuration(), effect.getLevel() - 1))),
			false, true);
	...
	public static final PotionEffectType MANA = new PotionEffectType(ChatColor.BLUE + "Mana", "mana",
			"Grants an instant %s Mana boost.", PotionColor.DARK_BLUE, ((effect, player) -> {
				Repeater.MANA_MAP.put(player.getUniqueId(),
						Repeater.MANA_MAP.get(player.getUniqueId())
								+ SUtil.getOrDefault(Arrays.asList(25, 50, 75, 100, 150, 200, 300, 400),
										effect.getLevel() - 1, (effect.getLevel() - 4) * 100));
			}), true, true);
	...
	public static final PotionEffectType ADRENALINE = new PotionEffectType(ChatColor.RED + "Adrenaline", "adrenaline",
			...
			(effect, player) -> PlayerUtils.replacePotionEffect(player,
					new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.ABSORPTION,
							(int) effect.getDuration(),
							(int) (SUtil.getOrDefault(Arrays.asList(20, 40, 60, 80, 100, 150, 200, 300),
									effect.getLevel() - 1, (effect.getLevel() - 5) * 100) * 0.25 - 1.0))),
			(statistics, slot, level) -> statistics.getSpeed().add(slot, (level * 5.0) / 100.0), false, true);
	...
	public static final PotionEffectType ABSORPTION = new PotionEffectType(ChatColor.GOLD + "Absorption", "absorption",
			...
			(effect, player) -> PlayerUtils
					.replacePotionEffect(player,
							new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.ABSORPTION,
									(int) effect.getDuration(),
									(int) (SUtil.getOrDefault(Arrays.asList(20, 40, 60, 80, 100, 150, 200, 300),
											effect.getLevel() - 1, (effect.getLevel() - 5) * 100) * 0.25 - 1.0))),
	...
	public static final PotionEffectType HASTE = new PotionEffectType(ChatColor.YELLOW + "Haste", "haste",
			"Increases your mining speed.", PotionColor.ORANGE, ((effect, player) -> {
				...
				player.addPotionEffect(
						new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.FAST_DIGGING,
								(int) effect.getDuration(), effect.getLevel() - 1));
			}), false, true);
	...
	public static final PotionEffectType RESISTANCE = new PotionEffectType(ChatColor.GREEN + "Resistance", "resistance",
			...
			((statistics, slot, level) -> statistics.getDefense().add(slot, SUtil.getOrDefault(
					Arrays.asList(5.0, 10.0, 15.0, 20.0, 30.0, 40.0, 50.0, 60.0), level - 1, (level * 10.0) - 20.0))),
			false, true);

	public static final PotionEffectType TRUE_RESISTANCE = new PotionEffectType(ChatColor.WHITE + "True Resistance",
			"true_resistance", "Increases True Defense by %s.", PotionColor.TWILIGHT_BLUE,
	...
	public static final PotionEffectType STAMINA = new PotionEffectType(ChatColor.GREEN + "Stamina", "stamina",
			"Grants an instant %s Health and %s Mana boost.", PotionColor.TWILIGHT_BLUE, ((effect, player) -> {
				player.setHealth(Math.min(player.getMaxHealth(),
						player.getHealth() + SUtil.getOrDefault(Arrays.asList(150, 215, 315, 515),
								effect.getLevel() - 1, ((effect.getLevel() + 1) * 100) + 15)));
				Repeater.MANA_MAP.put(player.getUniqueId(),
						Repeater.MANA_MAP.get(player.getUniqueId()) + (50 * effect.getLevel()));
			}), true, true);
	...
	public PotionEffectType(String name, String namespace, String description, PotionColor color,
			BiConsumer<PotionEffect, Player> onDrink, TriConsumer<PlayerStatistics, Integer, Integer> statsUpdate,
			boolean instant, boolean buff) {
		this.name = name;
	...
	public PotionEffectType(String name, String namespace, String description, PotionColor color, boolean instant,
			boolean buff) {
